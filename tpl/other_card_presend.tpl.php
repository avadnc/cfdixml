...
if ($action == 'presend')
{
    ...
    $filedir = $conf->cfdixml->multidir_output[$conf->entity].'/payment/'.dol_sanitizeFileName($object->ref).'/';
	...
	foreach($files as $file){                 
		if($file === '.' || $file === '..' || $file === '.DS_Store'){
			...
			$uuid = $object->array_options['options_cfdixml_UUID'];
			$explodedData = explode('_',$file);
			if(count($explodedData) == 2){
				$explodedUUID = explode('.',$explodedData[1]);				
				if($explodedUUID[0] == $uuid){
	...
	$formmail->param['doc_model'] = $doc_model;
	$formmail->param['models'] = $modelmail;
	$formmail->param['models_id'] = GETPOST('modelmailselected', 'int');
	$formmail->param['id'] = $object->id;
	$formmail->param['returnurl'] = $_SERVER["PHP_SELF"].'?id='.$object->id;
	//$formmail->param['fileinit'] = array($file);
	//$formmail->param['fileinit'] = $filesArray[0];
	// Show form
